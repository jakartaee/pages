...
public class XMLEncodingDetector {
    ...
    /*
     * PWC1.2 6298448 private boolean fAllowJavaEncodings;
     */
     ...
     *
     * Encoding autodetection is done according to the XML 1.0 specification, Appendix F.1: Detection Without External
     * Encoding Information.
     *
     * @return Two-element array, where the first element (of type java.lang.String) contains the name of the (auto)detected
     * encoding, and the second element (of type java.lang.Boolean) specifies whether the encoding was specified using the
     * 'encoding' attribute of an XML prolog (TRUE) or autodetected (FALSE).
     */
     ...
     * @param encoding The encoding name that the input stream is encoded using. If the user has specified that Java
     * encoding names are allowed, then the encoding name may be a Java encoding name; otherwise, it is an ianaEncoding
     * name.
     * @param isBigEndian For encodings (like uCS-4), whose names cannot specify a byte order, this tells whether the order
    ...
    /**
     * Returns the IANA encoding name that is auto-detected from the bytes specified, with the endian-ness of that encoding
     * where appropriate.
     *
     * @param b4 The first four bytes of the input.
     * @param count The number of bytes actually read.
     * @return a 2-element array: the first element, an IANA-encoding string, the second element a Boolean which is true iff
     * the document is big endian, false if it's little-endian, and null if the distinction isn't relevant.
     */
    private Object[] getEncodingName(byte[] b4, int count) {
        ...
        if (b0 == 0xFE && b1 == 0xFF) {
            ...
            return new Object[] { "UTF-16BE", Boolean.TRUE, Boolean.TRUE };
        }
        /*
         * SJSAS 6307968 if (b0 == 0xFF && b1 == 0xFE) {
         */
        // BEGIN SJSAS 6307968
        if (count == 2 && b0 == 0xFF && b1 == 0xFE) {
            // END SJSAS 6307968
            // UTF-16, little-endian, with a BOM
            return new Object[] { "UTF-16LE", Boolean.FALSE, Boolean.TRUE };
        }
        ...
        if (b0 == 0x00 && b1 == 0x00 && b2 == 0xFE && b3 == 0xFF) {
            ...
            return new Object[] { "UTF-32BE", Boolean.TRUE, Boolean.TRUE };
        }
        if (b0 == 0xFF && b1 == 0xFE && b2 == 0x00 && b3 == 0x00) {
            // UTF-32, little-endian, with a BOM
            return new Object[] { "UTF-32LE", Boolean.FALSE, Boolean.TRUE };
        }
        // BEGIN SJSAS 6307968
        if (b0 == 0xFF && b1 == 0xFE) {
            // UTF-16, little-endian, with a BOM
    ...
    /**
     * Returns a string matching the Name production appearing immediately on the input as a symbol, or null if no Name
     * string is present.
     * <p>
     * <strong>Note:</strong> The Name characters are consumed.
     * <p>
     * <strong>Note:</strong> The string returned must be a symbol. The SymbolTable can be used for this purpose.
     *
     * @throws IOException Thrown if i/o error occurs.
     * @throws EOFException Thrown on end of file.
     *
     ...
     * <p>
     * <strong>Note:</strong> This method does not guarantee to return the longest run of attribute value data. This method
     * may return before the quote character due to reaching the end of the input buffer or any other reason.
     * <p>
     * <strong>Note:</strong> The fields contained in the XMLString structure are not guaranteed to remain valid upon
     * subsequent calls to the entity scanner. Therefore, the caller is responsible for immediately using the returned
     * character data or making a copy of the character data.
     *
     * @param quote The quote character that signifies the end of the attribute value data.
     * @param content The content structure to fill.
     *
     * @return Returns the next character on the input, if known. This value may be -1 but this does <em>note</em> designate
     * end of file.
     *
     * @throws IOException Thrown if i/o error occurs.
     * @throws EOFException Thrown on end of file.
        ...
        if (c == '\n' || (c == '\r' && external)) {
            do {
                ...
                if (c == '\r' && external) {
                    ...
                    /***
                     * NEWLINE NORMALIZATION *** if (fCurrentEntity.ch[fCurrentEntity.position] == '\r' && external) {
    ...
    /**
     * Scans a range of character data up to the specified delimiter, setting the fields of the XMLString structure,
     * appropriately.
     * <p>
     * <strong>Note:</strong> The characters are consumed.
     * <p>
     * <strong>Note:</strong> This assumes that the internal buffer is at least the same size, or bigger, than the length of
     * the delimiter and that the delimiter contains at least one character.
     * <p>
     * <strong>Note:</strong> This method does not guarantee to return the longest run of character data. This method may
     * return before the delimiter due to reaching the end of the input buffer or any other reason.
     * <p>
     * <strong>Note:</strong> The fields contained in the XMLString structure are not guaranteed to remain valid upon
     * subsequent calls to the entity scanner. Therefore, the caller is responsible for immediately using the returned
     * character data or making a copy of the character data.
     ...
     * <p>
     * <strong>Note:</strong> The character is consumed only if it matches the specified character.
     *
     ...
     * <p>
     * <strong>Note:</strong> The characters are consumed only if they are space characters.
     *
        ...
        if (XMLChar.isSpace(c)) {
            ...
            do {
                ...
                if (c == '\n' || (external && c == '\r')) {
                    ...
                    /***
                     * NEWLINE NORMALIZATION *** else { if (fCurrentEntity.ch[fCurrentEntity.position + 1] == '\r' && external) {
                     * fCurrentEntity.position++; } } /
                     **/
                } else {
                    fCurrentEntity.columnNumber++;
     ...
     * @param changeEntity True if the load should change entities at the end of the entity, otherwise leave the current
     * entity in place and the entity boundary will be signaled by the return value.
     *
    ...
    /**
     * This class wraps the byte inputstreams we're presented with. We need it because java.io.InputStreams don't provide
     * functionality to reread processed bytes, and they have a habit of reading more than one character when you call their
     * read() methods. This means that, once we discover the true (declared) encoding of a document, we can neither
     * backtrack to read the whole doc again nor start reading where we are with a new reader.
     *
     * This class allows rewinding an inputStream by allowing a mark to be set, and the stream reset to that position.
     * <strong>The class assumes that it needs to read one character per invocation when it's read() method is inovked, but
     * uses the underlying InputStream's read(char[], offset length) method--it won't buffer data read this way!</strong>
     *
    ...
    private final class RewindableInputStream extends InputStream {
        ...
        public long skip(long n) throws IOException {
            ...
            n -= bytesLeft;
            /*
             * In a manner of speaking, when this class isn't permitting more than one byte at a time to be read, it is "blocking".
             * The available() method should indicate how much can be read without blocking, so while we're in this mode, it should
             * only indicate that bytes in its buffer are available; otherwise, the result of available() on the underlying
             * InputStream is appropriate.
             */
     ...
     *
     * @param scanningTextDecl True if a text declaration is to be scanned instead of an XML declaration.
     * @param pseudoAttributeValues An array of size 3 to return the version, encoding and standalone pseudo attribute
        ...
        while (peekChar() != '?') {
            ...
            case STATE_VERSION: {
                if (name == fVersionSymbol) {
                    if (!sawSpace) {
                        reportFatalError(
                                scanningTextDecl ? "jsp.error.xml.spaceRequiredBeforeVersionInTextDecl" : "jsp.error.xml.spaceRequiredBeforeVersionInXMLDecl",
                                null);
                    }
                    ...
                    if (!sawSpace) {
                        reportFatalError(
                                scanningTextDecl ? "jsp.error.xml.spaceRequiredBeforeEncodingInTextDecl" : "jsp.error.xml.spaceRequiredBeforeEncodingInXMLDecl",
                                null);
                    }
     ...
     *
     * @param scanningTextDecl True if scanning this pseudo-attribute for a TextDecl; false if scanning XMLDecl. This flag
     * is needed to report the correct type of error.
     * @param value The string to fill in with the attribute value.
     *
     * @return The name of the attribute
     *
     * <strong>Note:</strong> This method uses fStringBuffer2, anything in it at the time of calling is lost.
     */
    ...
    /**
     * Scans a processing data. This is needed to handle the situation where a document starts with a processing instruction
     * whose target name <em>starts with</em> "xml". (e.g. xmlfoo)
     *
     * <strong>Note:</strong> This method uses fStringBuffer, anything in it at the time of calling is lost.
     *
     ...
     * <p>
     * <strong>Note:</strong> This assumes the current char has already been identified as a high surrogate.
     *
