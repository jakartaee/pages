...
class Parser implements TagConstants {
     ...
     *
     * @param pc The ParseController, use for getting other objects in compiler and for parsing included pages
    ...
    public static Node.Nodes parse(ParserController pc, String path, JspReader reader, Node parent, boolean isTagFile, boolean directivesOnly, URL jarFileUrl,
            String pageEnc, String jspConfigPageEnc, boolean isDefaultPageEncoding, boolean hasBom) throws JasperException {
     ...
     * RTAttributeValueSingle | "'" AttributeValueSingle } Note: JSP and XML spec does not allow while spaces around Eq. It
     * is added to be backward compatible with Tomcat, and with other xml parsers.
     */
    ...
    /**
     * AttributeValueDouble ::= (QuotedChar - '"')* ('"' | <TRANSLATION_ERROR>) RTAttributeValueDouble ::= ((QuotedChar -
     * '"')* - ((QuotedChar-'"')'%>"') ('%>"' | TRANSLATION_ERROR)
     */
    ...
    private String parseQuoted(String tx) {
        ...
        while (i < size) {
            char ch = tx.charAt(i);
            if (ch == '&') {
                if (i + 5 < size && tx.charAt(i + 1) == 'a' && tx.charAt(i + 2) == 'p' && tx.charAt(i + 3) == 'o' && tx.charAt(i + 4) == 's'
                        && tx.charAt(i + 5) == ';') {
                    buf.append('\'');
                    i += 6;
                } else if (i + 5 < size && tx.charAt(i + 1) == 'q' && tx.charAt(i + 2) == 'u' && tx.charAt(i + 3) == 'o' && tx.charAt(i + 4) == 't'
                        && tx.charAt(i + 5) == ';') {
                    buf.append('"');
                    i += 6;
                } else {
    ...
    private void parsePageDirective(Node parent) throws JasperException {
        ...
        /*
         * A page directive may contain multiple 'import' attributes, each of which consists of a comma-separated list of
         * package names. Store each list with the node, where it is parsed.
         */
    ...
    private void parseTaglibDirective(Node parent) throws JasperException {
        ...
        if (prefix != null) {
            ...
            if (uri != null) {
                ...
                /*
                 * GlassFish 750 if (pageInfo.getTaglib(uri) == null) { String[] location = ctxt.getTldLocation(uri); TagLibraryInfoImpl
                 * taglib = null; try { taglib = new TagLibraryInfoImpl(ctxt, parserController, prefix, uri, location, err); } catch
                 * (JasperException je) { err.throwException(reader.mark(), je); } pageInfo.addTaglib(uri, taglib); }
                 */
                // START GlassFish 750
                ConcurrentHashMap<String, TagLibraryInfoImpl> taglibs = ctxt.getTaglibs();
                ...
                if (taglib == null) {
                    synchronized (taglibs) {
                        ...
                        if (taglib == null) {
                            ...
                            try {
                                taglib = new TagLibraryInfoImpl(ctxt, parserController, prefix, uri, location, err);
                ...
                if (pageInfo.getTaglib(uri) == null) {
                    pageInfo.addTaglib(uri, new TagLibraryInfoImpl(prefix, uri, taglib, pageInfo));
                }
                // END GlassFish 750
                ...
                if (tagdir != null) {
                    ...
                    if (pageInfo.getTaglib(urnTagdir) == null) {
                        pageInfo.addTaglib(urnTagdir, new ImplicitTagLibraryInfo(ctxt, parserController, prefix, tagdir, err));
                    }
    ...
    /*
     * Parses a directive with the following syntax: Directive ::= S? ( 'page' PageDirective | 'include' IncludeDirective |
     * 'taglib' TagLibDirective) S? '%>'
     *
    ...
    private void parseDirective(Node parent) throws JasperException {
        ...
        if (reader.matches("page")) {
            ...
            err.jspError(reader.mark(), "jsp.error.invalid.directive", reader.parseToken(false));
        }
    ...
    /*
     * ELExpressionBody (following "${" or "#{"to first unquoted "}") // XXX add formal production and confirm
     * implementation against it, // once it's decided
     */
    private void parseELExpression(Node parent, String typeEL) throws JasperException {
        start = reader.mark();
    ...
    /**
     * Attempts to parse 'JspAttributeAndBody' production. Returns true if it matched, or false if not. Assumes EmptyBody is
     * okay as well.
     *
    ...
    private boolean parseCustomTag(Node parent) throws JasperException {
        ...
        if (reader.matches("/>")) {
            if (tagInfo != null) {
                new Node.CustomTag(tagLibInfo.getRequiredVersion(), tagName, prefix, shortTagName, uri, attrs, start, parent, tagInfo, tagHandlerClass);
            } else {
                new Node.CustomTag(tagLibInfo.getRequiredVersion(), tagName, prefix, shortTagName, uri, attrs, start, parent, tagFileInfo);
            }
            return true;
        }
        ...
        Node tagNode = null;
        if (tagInfo != null) {
            tagNode = new Node.CustomTag(tagLibInfo.getRequiredVersion(), tagName, prefix, shortTagName, uri, attrs, start, parent, tagInfo, tagHandlerClass);
        } else {
            tagNode = new Node.CustomTag(tagLibInfo.getRequiredVersion(), tagName, prefix, shortTagName, uri, attrs, start, parent, tagFileInfo);
        }
    ...
    /*
     * XMLTemplateText ::= ( S? '/>' ) | ( S? '>' ( ( Char* - ( Char* ( '<' | '${' ) ) ) ( '${' ELExpressionBody )? CDSect?
     ...
     * DeclarationBody ) | ( '<jsp:declaration' XMLDeclarationBody ) | ( '<%=' ExpressionBody ) | ( '<jsp:expression'
     * XMLExpressionBody ) | ( '${' ELExpressionBody ) | ( '<%' ScriptletBody ) | ( '<jsp:scriptlet' XMLScriptletBody ) | (
     * '<jsp:text' XMLTemplateText ) | ( '<jsp:' StandardAction ) | ( '<' CustomAction CustomActionBody ) | TemplateText
     */
     ...
     * '<%!' <TRANSLATION_ERROR> ) | ( '<jsp:declaration' <TRANSLATION_ERROR> ) | ( '<%=' <TRANSLATION_ERROR> ) | (
     * '<jsp:expression' <TRANSLATION_ERROR> ) | ( '<%' <TRANSLATION_ERROR> ) | ( '<jsp:scriptlet' <TRANSLATION_ERROR> ) | (
     * '<jsp:text' <TRANSLATION_ERROR> ) | ( '${' <TRANSLATION_ERROR> ) | ( '<jsp:' <TRANSLATION_ERROR> ) | TemplateText
     */
    ...
    private void parseBody(Node parent, String tag, String bodyType) throws JasperException {
        if (bodyType.equalsIgnoreCase(TagInfo.BODY_CONTENT_TAG_DEPENDENT)) {
        ...
        } else if (bodyType.equalsIgnoreCase(TagInfo.BODY_CONTENT_JSP) || bodyType.equalsIgnoreCase(TagInfo.BODY_CONTENT_SCRIPTLESS)
                || (bodyType == JAVAX_BODY_CONTENT_PARAM) || (bodyType == JAVAX_BODY_CONTENT_TEMPLATE_TEXT)) {
            while (reader.hasMoreInput()) {
                if (reader.matchesETag(tag)) {
                    return;
                }
            ...
            err.jspError(start, "jsp.error.tld.badbodycontent", bodyType, tag);
        }
    }
