...
class JspDocumentParser extends DefaultHandler implements LexicalHandler, TagConstants {
    ...
    private static final EnableDTDValidationException ENABLE_DTD_VALIDATION_EXCEPTION = new EnableDTDValidationException("jsp.error.enable_dtd_validation",
            null);
    ...
    /*
     * Outermost (in the nesting hierarchy) node whose body is declared to be scriptless. If a node's body is declared to be
     * scriptless, all its nested nodes must be scriptless, too.
     */
     ...
     */
    public static Node.Nodes parse(ParserController pc, String path, JarFile jarFile, Node parent, boolean isTagFile, boolean directivesOnly, String pageEnc,
            String jspConfigPageEnc, boolean isEncodingSpecifiedInProlog) throws JasperException {
     ...
     *
     * The SAX does not call this method with all of the template text, but may invoke this method with chunks of it. This
     * is a problem when we try to determine if the text contains only whitespaces, or when we are looking for an EL
     * expression string. Therefore it is necessary to buffer and concatenate the chunks and process the concatenated text
    ...
    private void processChars() throws SAXException {
        ...
        /*
         * JSP.6.1.1: All textual nodes that have only white space are to be dropped from the document, except for nodes in a
         * jsp:text element, and any leading and trailing white-space-only textual nodes in a jsp:attribute whose 'trim'
         * attribute is set to FALSE, which are to be kept verbatim. JSP.6.2.3 defines white space characters.
         */
                ...
                if ((lastCh == '$' || lastCh == '#') && ch == '{') {
                    ...
                    for (;; i++) {
                        if (i >= charBuffer.length()) {
                            throw new SAXParseException(Localizer.getMessage("jsp.error.unterminated", lastCh == '$' ? "${" : "#{"), locator);
                        ...
                        if (ch == '}') {
                            ttext.write('}');
                            new Node.ELExpression(ttext.toString(), startMark, current);
                            ttext = new CharArrayWriter();
    ...
    private Node parseStandardAction(String qName, String localName, Attributes nonTaglibAttrs, Attributes nonTaglibXmlnsAttrs, Attributes taglibAttrs,
     ...
     */
    private Node parseCustomAction(String qName, String localName, String uri, Attributes nonTaglibAttrs, Attributes nonTaglibXmlnsAttrs,
            Attributes taglibAttrs, Mark start, Node parent) throws SAXException {
        ...
        if (tagInfo != null) {
            ret = new Node.CustomTag(tagLibInfo.getRequiredVersion(), qName, prefix, localName, uri, nonTaglibAttrs, nonTaglibXmlnsAttrs, taglibAttrs, start,
    ...
    /*
     * Adds the tag library associated with the given uri namespace to the translation unit that this parser is associated
     * with.
     *
    ...
    private void addTaglibInfo(String prefix, String uri) throws JasperException {
        if (uri.startsWith(URN_JSPTAGDIR)) {
            ...
            TagLibraryInfo taglibInfo = new ImplicitTagLibraryInfo(ctxt, parserController, prefix, tagdir, err);
                ...
                synchronized (taglibs) {
                    ...
                    if (taglibInfo == null) {
                        ...
                        if (location != null || !isPlainUri) {
                            /*
                             * If the uri value is a plain uri, a translation error must not be generated if the uri is not found in the taglib map.
                             * Instead, any actions in the namespace defined by the uri value must be treated as uninterpreted.
                             */
                            taglibInfo = new TagLibraryInfoImpl(ctxt, parserController, prefix, uri, location, err);
                            // START GlassFish 750
            ...
            if (taglibInfo != null) {
                if (pageInfo.getTaglib(uri) == null) {
                    pageInfo.addTaglib(uri, new TagLibraryInfoImpl(prefix, uri, taglibInfo, pageInfo));
                }
                pageInfo.pushPrefixMapping(prefix, uri);
            } else {
     ...
     *
     * This check is performed only for the body of a scripting (that is: declaration, scriptlet, or expression) element,
     * after the end tag of a scripting element has been reached.
     */
    ...
    /*
     * Checks an element's given URI, qname, and attributes to see if any of them hijack the 'jsp' prefix, that is, bind it
    ...
    /*
     * Checks the given URI and qname to see if they hijack the 'jsp' prefix, which would be the case if qName contained the
