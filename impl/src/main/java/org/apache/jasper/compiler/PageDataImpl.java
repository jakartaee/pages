...
/**
 * An implementation of <tt>jakarta.servlet.jsp.tagext.PageData</tt> which builds the XML view of a given page.
 *
 * The XML view is built in two passes:
 *
 * During the first pass, the FirstPassVisitor collects the attributes of the top-level jsp:root and those of the
 * jsp:root elements of any included pages, and adds them to the jsp:root element of the XML view. In addition, any
 * taglib directives are converted into xmlns: attributes and added to the jsp:root element of the XML view. This pass
 * ignores any nodes other than JspRoot and TaglibDirective.
 *
 * During the second pass, the SecondPassVisitor produces the XML view, using the combined jsp:root attributes
 * determined in the first pass and any remaining pages nodes (this pass ignores any JspRoot and TaglibDirective nodes).
 *
 * @author Jan Luehe
 */
class PageDataImpl extends PageData implements TagConstants {
    ...
    public PageDataImpl(Node.Nodes page, Compiler compiler) throws JasperException {
        ...
        // First pass
        FirstPassVisitor firstPass = new FirstPassVisitor(page.getRoot(), compiler.getPageInfo(), compiler.getErrorDispatcher());
    ...
    /*
     * First-pass Visitor for JspRoot nodes (representing jsp:root elements) and TablibDirective nodes, ignoring any other
     * nodes.
     *
     * The purpose of this Visitor is to collect the attributes of the top-level jsp:root and those of the jsp:root elements
     * of any included pages, and add them to the jsp:root element of the XML view. In addition, this Visitor converts any
     * taglib directives into xmlns: attributes and adds them to the jsp:root element of the XML view.
     */
    static class FirstPassVisitor extends Node.Visitor implements TagConstants {
        ...
        public void visit(Node.Root n) throws JasperException {
            ...
            if (n == root) {
                ...
                if (pageInfo.isJspPrefixHijacked()) {
                    /*
                     * 'jsp' prefix has been hijacked, that is, bound to a namespace other than the JSP namespace. This means that when
                     * adding an 'id' attribute to each element, we can't use the 'jsp' prefix. Therefore, create a new prefix (one that is
                     * unique across the translation unit) for use by the 'id' attribute, and bind it to the JSP namespace
                     */
                    jspIdPrefix += "jsp";
        ...
        public void visit(Node.TaglibDirective n) throws JasperException {
            ...
            if (attrs != null) {
                ...
                /*
                 * According to javadocs of org.xml.sax.helpers.AttributesImpl, the addAttribute method does not check to see if the
                 * specified attribute is already contained in the list: This is the application's responsibility!
                 */
        ...
        private void addRootAttributes(Attributes attrs) throws JasperException {
            if (attrs != null) {
                ...
                for (int i = 0; i < len; i++) {
                    ...
                    if (index == -1) {
                        rootAttrs.addAttribute(attrs.getURI(i), attrs.getLocalName(i), qname, attrs.getType(i), value);
                    } else {
                        String rootValue = rootAttrs.getValue(index);
                        if (rootValue != null && !rootValue.equals(value)) {
                            err.jspError("jsp.error.xmlview.attribute.redefined", qname, rootValue, value);
                        }
                    }
                }
            }
    ...
    static class SecondPassVisitor extends Node.Visitor implements TagConstants {
        ...
        /*
         * Appends the given tag, including its body, to the XML view, and optionally reset default namespace to "", if none
         * specified.
         */
         ...
         *
         * Since the import attribute of the page directive is the only page attribute that is allowed to appear multiple times
         * within the same document, and since XML allows only single-value attributes, the values of multiple import attributes
         * must be combined into one, separated by comma.
         *
         * If the given page directive contains just 'contentType' and/or 'pageEncoding' attributes, we ignore it, as we've
         * already appended a page directive containing just these two attributes.
         */
        private void appendPageDirective(Node.PageDirective n) {
            ...
            for (int i = 0; i < len; i++) {
                ...
                if ("import".equals(attrName) || "contentType".equals(attrName) || "pageEncoding".equals(attrName)) {
                    /*
                     * Page directive's 'import' attribute is considered further down, and its 'pageEncoding' and 'contentType' attributes
                     * are ignored, since we've already appended a new page directive containing just these two attributes
                     */
                    continue;
                }
                String value = attrs.getValue(i);
         ...
         *
         * The value of the 'pageEncoding' attribute is hard-coded to UTF-8, whereas the value of the 'contentType' attribute,
         * which is identical to what the container will pass to ServletResponse.setContentType(), is derived from the pageInfo.
         */
         ...
         *
         * If the given tag directive contains just a 'pageEncoding' attributes, we ignore it, as we've already appended a tag
         * directive containing just this attributes.
         */
        ...
        /*
         * Escapes any occurrences of "]]>" (by replacing them with "]]&gt;") within the given text, so it can be included in a
         * CDATA section.
         */
