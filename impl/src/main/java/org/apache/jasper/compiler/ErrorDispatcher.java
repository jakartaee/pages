...
public class ErrorDispatcher {
     ...
     *
     * The given error code is localized. If it is not found in the resource bundle for localized error messages, it is used
    ...
    /*
     * Parses the given Java compilation error message, which may contain one or more compilation errors, into an array of
     * JavacErrorDetail instances.
     *
    ...
    public static JavacErrorDetail[] parseJavacMessage(Node.Nodes pageNodes, String errMsg, String fname) throws IOException, JasperException {
        ...
        /*
         * Parse compilation errors. Each compilation error consists of a file path and error line number, followed by a number
         * of lines describing the error.
         */
        String line = null;
        while ((line = reader.readLine()) != null) {

            /*
             * Error line number is delimited by set of colons. Ignore colon following drive letter on Windows (fromIndex = 2). XXX
             * Handle deprecation warnings that don't have line info
             */
            ...
            if ((beginColon >= 0) && (endColon >= 0)) {
                ...
                javacError = createJavacError(fname, pageNodes, errMsgBuf, lineNum);
            }
    ...
    public static JavacErrorDetail createJavacError(String fname, Node.Nodes page, StringBuilder errMsgBuf, int lineNum)
        ...
        if ((errNode != null) && (errNode.getStart() != null)) {
            javacError = new JavacErrorDetail(fname, lineNum, errNode.getStart().getFile(), errNode.getStart().getLineNumber(), errMsgBuf);
        } else {
            /*
             * javac error line number cannot be mapped to JSP page line number. For example, this is the case if a scriptlet is
             * missing a closing brace, which causes havoc with the try-catch-finally block that the code generator places around
             * all generated code: As a result of this, the javac error line numbers will be outside the range of begin and end java
             * line numbers that were generated for the scriptlet, and therefore cannot be mapped to the start line number of the
             * scriptlet in the JSP page. Include just the javac error info in the error detail.
             */
