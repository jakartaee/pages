...
class Validator {
    ...
    static class DirectiveVisitor extends Node.Visitor {
        ...
        public void visit(Node.PageDirective n) throws JasperException {
            ...
            for (int i = 0; attrs != null && i < attrs.getLength(); i++) {
                ...
                if ("language".equals(attr)) {
                    ...
                    /*
                     * Report any encoding conflict, treating "UTF-16", "UTF-16BE", and "UTF-16LE" as identical.
                     */
                    ...
                    if (pageInfo.getDeferredSyntaxAllowedAsLiteral() == null) {
                        ...
                        err.jspError(n, "jsp.error.page.conflict.deferred", pageInfo.getDeferredSyntaxAllowedAsLiteral(), value);
                    }
            ...
            if (pageInfo.isErrorPage() && pageInfo.getErrorPage() != null) {
                ...
                if (!errorPath.startsWith("/")) {
                    ...
                    String baseRootPath = rootPath.substring(0, rootPath.lastIndexOf('/'));
                    errorPath = baseRootPath + '/' + errorPath;
                }
                if (rootPath.equals(errorPath)) {
                    err.jspError(n, "jsp.error.page.selfreferencing", rootPath);
                }
            }
            ...
            for (int i = 0; attrs != null && i < attrs.getLength(); i++) {
                ...
                if ("language".equals(attr)) {
                    ...
                    if (n.getRoot().hasBom()) {
                        ...
                        // identical.
                        if (value != null && !value.equalsIgnoreCase(bom)
                                && (!value.toLowerCase().startsWith("utf-16") || !bom.toLowerCase().startsWith("utf-16"))) {
                            err.jspError(n, "jsp.error.bom_tagdir_encoding_mismatch", bom, value);
                        }
                    }
                    ...
                    if (pageInfo.getDeferredSyntaxAllowedAsLiteral() == null) {
                        ...
                        err.jspError(n, "jsp.error.tag.conflict.deferred", pageInfo.getDeferredSyntaxAllowedAsLiteral(), value);
                    }
                } else if ("trimDirectiveWhitespaces".equals(attr)) {
                    if (pageInfo.getTrimDirectiveWhitespaces() == null) {
        ...
        /*
         * Ensures that the page encoding specified in the 'pageEncoding' attribute of the page directive matches the page
         * encodings specified in other applicable authoritative page encoding sources.
         *
         * @param pageDirEnc The value of the pageEncoding attribute of the page directive
         *
        ...
        private void comparePageEncodings(String pageDirEnc, Node.PageDirective pageDir) throws JasperException {
            ...
            /*
             * Compare the 'pageEncoding' attribute of the page directive with the encoding specified in the JSP config element
             * whose URL pattern matches this page. Treat "UTF-16", "UTF-16BE", and "UTF-16LE" as identical.
             */
            if (configEnc != null && !pageDirEnc.equalsIgnoreCase(configEnc)
                    && (!pageDirEnc.toLowerCase().startsWith("utf-16") || !configEnc.toLowerCase().startsWith("utf-16"))) {
                err.jspError(pageDir, "jsp.error.config_pagedir_encoding_mismatch", configEnc, pageDirEnc);
            }

            /*
             * Compare the 'pageEncoding' attribute of the page directive with the encoding specified in the XML prolog (only for
             * XML syntax, and only if JSP document contains XML prolog with encoding declaration). Treat "UTF-16", "UTF-16BE", and
             * "UTF-16LE" as identical.
             */
            if (root.isXmlSyntax() && root.isEncodingSpecifiedInProlog()) {
                String pageEnc = root.getPageEncoding();
                if (!pageDirEnc.equalsIgnoreCase(pageEnc) && (!pageDirEnc.toLowerCase().startsWith("utf-16") || !pageEnc.toLowerCase().startsWith("utf-16"))) {
                    err.jspError(pageDir, "jsp.error.prolog_pagedir_encoding_mismatch", pageEnc, pageDirEnc);
                }
            }

            /*
             * Compare the 'pageEncoding' attribute of the page directive with the encoding declared by BOM (if present). Treat
             * "UTF-16", "UTF-16BE", and "UTF-16LE" as identical.
             */
            if (root.hasBom()) {
                String pageEnc = root.getPageEncoding();
                if (!pageDirEnc.equalsIgnoreCase(pageEnc) && (!pageDirEnc.toLowerCase().startsWith("utf-16") || !pageEnc.toLowerCase().startsWith("utf-16"))) {
                    err.jspError(pageDir, "jsp.error.bom_pagedir_encoding_mismatch", pageEnc, pageDirEnc);
                }
            }
        }
    }
    ...
    static class ValidateVisitor extends Node.Visitor {
        ...
        public void visit(Node.ParamAction n) throws JasperException {
            ...
            n.setValue(getJspAttribute("value", null, null, n.getAttributeValue("value"), n, false, null));
            visitBody(n);
        }
        ...
        public void visit(Node.SetProperty n) throws JasperException {
            ...
            n.setValue(getJspAttribute("value", null, null, value, n, false, null));
        ...
        public void visit(Node.CustomTag n) throws JasperException {
            ...
            /*
             * Make sure all required attributes are present, either as attributes or named attributes (<jsp:attribute>). Also make
        ...
        /*
         * Check that the setter method exists for the tag handler and is consistent with the TLD entries.
         *
        ...
        private void checkSetter(Node.CustomTag n, TagAttributeInfo tldattr) throws JasperException {
             ...
             * tldattr.getTypeName(); if (tldType != null && ! tldType.equals(typeName)) { err.jspError(n,
             * "jsp.error.setter.notequal", handlerName, property); }
             */
        }
         ...
         *
         * A custom action and its declared attributes always belong to the same namespace, which is identified by the prefix
         * name of the custom tag invocation. For example, in this invocation:
         *
         * <my:test a="1" b="2" c="3"/>, the action
         *
         * "test" and its attributes "a", "b", and "c" all belong to the namespace identified by the prefix "my". The above
         * invocation would be equivalent to:
         *
                            ...
                            if (el != null) {
                                if (el.hasDollarExpression()) {
                                    ...
                                    if (isLiteral) {
                                        ...
                                        jspAttrs[i].setValue(escapePound(jspAttrs[i].getValue()));
                                    }
        ...
        private void checkNamedAttributes(Node.CustomTag n, Node.JspAttribute[] jspAttrs, int start, Hashtable<String, Object> tagDataAttrs)
            ...
            for (int i = 0; i < naNodes.size(); i++) {
                ...
                for (int j = 0; j < tldAttrs.length; j++) {
                    /*
                     * See above comment about namespace matches. For named attributes, we use the prefix instead of URI as the match
                     * criterion, because in the case of a JSP document, we'd have to keep track of which namespaces are in scope when
                     * parsing a named attribute, in order to determine the URI that the prefix of the named attribute's name matches to.
                     */
         ...
         * <p>
         * If value is null, checks if there are any NamedAttribute subelements in the tree node, and if so, constructs a
         * JspAttribute out of a child NamedAttribute node.
         */
        private Node.JspAttribute getJspAttribute(String qName, String uri, String localName, String value, Node n, boolean dynamic, TagAttributeInfo tagAttr)
                throws JasperException {
                ...
                if (n.getRoot().isXmlSyntax() && value.startsWith("%=")) {
                    ...
                    if (el.hasPoundExpression() && tagAttr == null && !dynamic) {
                        ...
                        if (pageInfo.isELIgnored() || pageInfo.isDeferredSyntaxAllowedAsLiteral()) {
                            result = new Node.JspAttribute(qName, uri, localName, getLiteral(n, value), false, null, false);
                        } else {
                            err.jspError(n, "jsp.error.el.action.pound");
                        }
                    } else if ((el.containsEL() && !pageInfo.isELIgnored())
                            || ((tagAttr != null) && ((tagAttr.isDeferredValue() && !tagAttr.canBeRequestTime()) || tagAttr.isDeferredMethod()))) {
                        validateFunctions(el, n);
                        JspUtil.validateExpressions(n.getStart(), value, getFunctionMapper(el), this.err);
                        ...
                        if (tagAttr != null && tagAttr.isDeferredValue()) {
                            ...
                            if (!el.containsEL()) {
                                try {
                                    Class typeClass = JspUtil.toClass(returnType, loader);
                                    JspUtil.coerce(typeClass, value);
                                } catch (ClassNotFoundException ex) {
                                    err.jspError(n, "jsp.error.el.method.type", qName, returnType);
                                } catch (Exception ex) {
                                    err.jspError(n, "jsp.error.el.method.literal", qName);
                                }
                            }

                            result = new Node.JspAttribute(qName, uri, localName, value, el, null, getReturnType(methodSignature),
                                    getParameters(methodSignature));
                        } else {
                            result = new Node.JspAttribute(qName, uri, localName, value, false, el, dynamic);
                        }
                    } else {
                        result = new Node.JspAttribute(qName, uri, localName, getLiteral(n, value), false, null, dynamic);
                    }
                }
            } else {
        ...
        /*
         * Checks to see if the given attribute value represents a string or an expression (runtime or EL). If it is a literal,
         * also process escape sequences as a side-effect. Note that the treatment of the EL syntax "#{}" depends on the JSP
         * version in the TLD.
         *
         *
        ...
        /*
         * Since the EL engine reconginizes escape sequence for #{ }, literals sent to it must be properly escaped, so "#{}"
            ...
            class MapperELVisitor extends ELNode.Visitor {
                ...
                public void visit(ELNode.Function n) throws JasperException {
                    ...
                    try {
                        ...
                        err.jspError("jsp.error.function.classnotfound", n.getFunctionInfo().getFunctionClass(), n.getPrefix() + ':' + n.getName(),
                                e.getMessage());
                    }
                    ...
                    if (!Modifier.isPublic(method.getModifiers())) {
                        err.jspError("jsp.error.nonPublicFunction", c.getName() + '.' + method.getName());
                    }
                    // Make sure that this method is static
                    if (!Modifier.isStatic(method.getModifiers())) {
                        err.jspError("jsp.error.nonStaticFunction", c.getName() + '.' + method.getName());
                    }
    ...
    public static void validate(Compiler compiler, Node.Nodes page) throws JasperException {
        ...
        /*
         * Visit the page/tag directives first, as they are global to the page and are position independent.
         */
        ...
        /*
         * Validate all other nodes. This validation step includes checking a custom tag's mandatory and optional attributes
         * against information in the TLD (first validation step for custom tags according to JSP.10.5).
         */
        ...
        /*
         * Invoke TagExtraInfo method isValid() for all imported tags (third validation step for custom tags according to
         * JSP.10.5).
         */
