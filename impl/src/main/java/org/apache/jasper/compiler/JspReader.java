...
class JspReader {
    ...
    /*
     * Set to true when using the JspReader on a single file where we read up to the end and reset to the beginning many
     * times. (as in ParserController.figureOutJspDocument()).
     */
     ...
     * @return <strong>true</strong> is one is found, the current position in stream is positioned after the search string,
     * <strong> false</strong> otherwise, position in stream unchanged.
     */
     ...
     * spaces and characters are skipped. If not, false is returned and the position is restored to where we were before.
     */
    ...
    /**
     * Skip until the given string is matched in the stream. When returned, the context is positioned past the end of the
     * match.
     *
    ...
    /**
     * Skip until the given string is matched in the stream, but ignoring chars initially escaped by a '\'. When returned,
    ...
    /**
     * Skip until the given end tag is matched in the stream. When returned, the context is positioned past the end of the
     * tag.
     *
    ...
    /**
     * Parse a space delimited token. If quoted the token will consume all characters up to a matching quote, otherwise, it
     * consumes up to the first delimiter character.
     *
     ...
     *
     * @exception MalformedURLException if the path name is not given in the correct form
     */
    ...
    /**
     * Parse utils - Is current character a token delimiter ? Delimiters are currently defined to be =, &gt;, &lt;, ", and '
     * or any any space character as defined by <code>isSpace</code>.
     *
     * @return A boolean.
     */
    ...
    /**
     * Register a new source file. This method is used to implement file inclusion. Each included file gets a unique
     * identifier (which is the index in the array of source files).
     *
    ...
    /**
     * Unregister the source file. This method is used to implement file inclusion. Each included file gets a uniq
     * identifier (which is the index in the array of source files).
     *
     * @return The index of the now registered file.
     */
    ...
    /**
     * Pop a file from the file stack. The field "current" is retored to the value to point to the previous files, if any,
     * and is set to null otherwise.
     *
     * @return true is there is a previous file on the stck. false otherwise.
     */
