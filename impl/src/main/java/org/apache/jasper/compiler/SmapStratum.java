...
public class SmapStratum {
    ...
    public static class LineInfo {
        ...
        /**
         * Sets lineFileID. Should be called only when different from that of prior LineInfo object (in any given context) or 0
        ...
        /**
         * Retrieves the current LineInfo as a String, print all values only when appropriate (but LineInfoID if and only if
    ...
    public void optimizeLineSection() {
        ...
        while (i < lineData.size() - 1) {
            ...
            if (!liNext.lineFileIDSet && liNext.inputStartLine == li.inputStartLine && liNext.inputLineCount == 1 && li.inputLineCount == 1
        ...
        while (i < lineData.size() - 1) {
            LineInfo li = lineData.get(i);
            LineInfo liNext = lineData.get(i + 1);
            if (!liNext.lineFileIDSet && liNext.inputStartLine == li.inputStartLine + li.inputLineCount && liNext.outputLineIncrement == li.outputLineIncrement
                    && liNext.outputStartLine == li.outputStartLine + li.inputLineCount * li.outputLineIncrement) {
    ...
    /**
     * Adds complete information about a simple line mapping. Specify all the fields in this method; the back-end machinery
     * takes care of printing only those that are necessary in the final SMAP. (My view is that fields are optional
     * primarily for spatial efficiency, not for programmer convenience. Could always add utility methods later.)
     *
     * @param inputStartLine starting line in the source file (SMAP <tt>InputStartLine</tt>)
     * @param inputFileName the filepath (or name) from which the input comes (yields SMAP <tt>LineFileID</tt>) Use
     * unqualified names carefully, and only when they uniquely identify a file.
     * @param inputLineCount the number of lines in the input to map (SMAP <tt>LineFileCount</tt>)
     * @param outputStartLine starting line in the output file (SMAP <tt>OutputStartLine</tt>)
     * @param outputLineIncrement number of output lines to map to each input line (SMAP <tt>OutputLineIncrement</tt>).
     * <i>Given the fact that the name starts with "output", I continuously have the subconscious urge to call this field
    ...
    /**
     * Returns the given stratum as a String: a StratumSection, followed by at least one FileSection and at least one
     * LineSection.
     */
    public String getString() {
