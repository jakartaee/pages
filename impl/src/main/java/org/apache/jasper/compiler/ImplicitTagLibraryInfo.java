...
class ImplicitTagLibraryInfo extends TagLibraryInfo {
        ...
        if (dirList != null) {
            ...
            while (it.hasNext()) {
                ...
                if (path.endsWith(TAG_FILE_SUFFIX) || path.endsWith(TAGX_FILE_SUFFIX)) {
                    /*
                     * Use the filename of the tag file, without the .tag or .tagx extension, respectively, as the <name> subelement of the
                     * "imaginary" <tag-file> element
                     */
    ...
    private void parseImplicitTld(JspCompilationContext ctxt, String path) throws JasperException {
        ...
        try {
            ...
            is = uri.openStream();
            /*
             * SJSAS 6384538 tld = new ParserUtils().parseXMLDocument(IMPLICIT_TLD, is);
             */
            // START SJSAS 6384538
            ...
            tld = new ParserUtils(blockExternal).parseXMLDocument(IMPLICIT_TLD, is, ctxt.getOptions().isValidationEnabled());
            // END SJSAS 6384538
        } catch (Exception ex) {
