...
class ParserController implements TagConstants {
     ...
     * @param jarFile The JAR file from which to read the included resource, or null of the included resource is to be read
     * from the filesystem
     */
     ...
     * @param isTagFile true if file to be parsed is tag file, and false if it is a regular JSP page
     * @param directivesOnly true if the file to be parsed is a tag file and we are only interested in the directives needed
     * for constructing a TagFileInfo.
     * @param jarFile The JAR file from which to read the JSP page or tag file, or null if the JSP page or tag file is to be
    ...
    private Node.Nodes doParse(String inFileName, Node parent, URL jarFileUrl) throws FileNotFoundException, JasperException, IOException {
        ...
        if (isXml) {
            ...
            // JspDocumentParser.
            parsedPage = JspDocumentParser.parse(this, absFileName, jarFile, parent, isTagFile, directiveOnly, sourceEnc, jspConfigPageEnc,
            ...
            try {
                ...
                parsedPage = Parser.parse(this, absFileName, jspReader, parent, isTagFile, directiveOnly, jarFileUrl, sourceEnc, jspConfigPageEnc,
    ...
    /*
     * Ensures that the page encoding specified in the JSP config element (with matching URL pattern), if present, matches
     * the page encoding specified in the XML prolog of a JSP document (XML syntax) and the page encoding derived from the
     * BOM.
     *
     ...
     */
    private void comparePageEncodings(String jspConfigPageEnc) throws JasperException {
        ...
        if (isXml && isEncodingSpecifiedInProlog) {
            /*
             * Make sure the encoding specified in the XML prolog matches that in the JSP config element, treating "UTF-16",
    ...
    /*
     * Checks to see if the given URI is matched by a URL pattern specified in a jsp-property-group in web.xml, and if so,
     * returns the value of the <page-encoding> element.
     *
    ...
    private void determineSyntaxAndEncoding(String absFileName, JarFile jarFile, String jspConfigPageEnc) throws JasperException, IOException {
        ...
        /*
         * 'true' if the syntax (XML or standard) of the file is given from external information: either via a JSP configuration
         * element, the ".jspx" suffix, or the enclosing file (for included resources)
         */
        ...
        if (isExternal && !isXml) {
            ...
            if (!isXml && sourceEnc.equalsIgnoreCase("utf-8") && !hasBom) {
                /*
                 * We don't know if we're dealing with XML or standard syntax. Therefore, we need to check to see if the page contains a
                 * <jsp:root> element.
                 *
                 * We need to be careful, because the page may be encoded in ISO-8859-1 (or something entirely different: UTF-8 was
                 * chosen as the default, for lack of better alternative), and may contain byte sequences that will cause a UTF-8
                 * converter to throw exceptions.
                 *
                 * It is safe to use a source encoding of ISO-8859-1 in this case, as there are no invalid byte sequences in ISO-8859-1,
                 * and the byte/character sequences we're looking for (i.e., <jsp:root>) are identical in either encoding (both UTF-8
        ...
        /*
         * At this point, 'isExternal' or 'isXml' is FALSE. Search for jsp:root action, in order to determine if we're dealing
         * with XML or standard syntax (unless we already know what we're dealing with, i.e., when 'isExternal' is TRUE and
         * 'isXml' is FALSE). No check for XML prolog, since nothing prevents a page from outputting XML and still using JSP
        ...
        /*
         * At this point, we know we're dealing with JSP syntax. If an XML prolog is provided, it's treated as template text.
         * Determine the page encoding from the page directive, unless it's specified via JSP config.
         */
        if (!hasBom) {
            sourceEnc = jspConfigPageEnc;
        }
    ...
    /*
     * Scans the given attributes for the attribute with the given name, which is either 'pageEncoding' or 'contentType',
     * and returns the specified page encoding.
     *
    ...
    /*
     * Resolve the name of the file and update baseDirStack() to keep track of the current base directory for each included
     * file. The 'root' file is always an 'absolute' path, so no need to put an initial value in the baseDirStack.
     */
    ...
    private boolean hasJspRoot(JspReader reader) throws JasperException {
        ...
        if (index < root.length() && root.charAt(index) == '=') {
            ...
            if (index < root.length() && root.charAt(index++) == '"' && root.regionMatches(index, JSP_URI, 0, JSP_URI.length())) {
                return true;
            }
        }
