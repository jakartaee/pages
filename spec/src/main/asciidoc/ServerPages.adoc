...
<page-encoding>Shift`JIS</page-encoding> +
</jsp-property-group>
...
*  `A.jsp` says `<%@ include
file=”dir/B.jsp”%>` and `dir/B.jsp` says `<%@ include file=”C.jsp”%>` .
...
of either `AT`BEGIN` or `NESTED` . The container must then generate code
...
=== The interpretation of a `jsp:text` element is to pass its content through to the current value of `out` . This is very similar to the XSLT `xsl:text` element.Examples
...
The <jsp:output> action has the following syntax:
 ...
 `<jsp:output (
omit-xml-declaration=”yes|no|true|false” ) \{ doctypeDecl } />`
...
Where `page`directive`attr`list` is as
...
<title>$\{title}</title> +
 ...
 `where pageCharSet is a variable with a
value as iso-8859-1.`
...
*  `Portable` - An action described in a tag
library must be usable in any JSP container.
*  `Simple` - Unsophisticated users must be
...
*  `Expressive` - The mechanism must support
...
variables.
*  `Usable from different scripting
languages` - Although the JSP specification currently only defines the
semantics for scripts in the Java programming language, we want to leave
open the possibility of other scripting languages.
*  `Built upon existing concepts and
machinery` - We do not want to reinvent what exists elsewhere. Also, we
...
<%@ taglib
uri=”/WEB-INF/tlds/PRlibrary`1`4.tld” prefix=”x” %>
...
http://java.sun.com/xml/ns/j2ee/web-jsptaglibrary`2`1.xsd +
...
The corresponding `acme.xsd` file would look
something like:
...
synchronizing the `AT`BEGIN` , `NESTED` , and `AT`END` scripting
...
<%@ tag tag`directive`attr`list %> +
 +
tag`directive`attr`list ::= +
...
=== [[a2408]]Details of `tag` directive attributes
...
<%@ attribute attribute`directive`attr`list
%> +
 +
attribute`directive`attr`list ::= +
name=”attribute-name” +
\{ required=”true|false” } +
\{ fragment=”true|false” } +
...
at `j` `avax.servlet.jsp.tagext.JspFragment` and a translation error
...
<%@ variable variable`directive`attr`list
%> +
 +
variable`directive`attr`list ::= +
...
variable defined. Can be either `AT`BEGIN` , `AT`END` , or `NESTED` .
Defaults to `NESTED` .
...
(as in the call `getServletConfig(). +
getContext()` )
...
`AT`BEGIN` or `AT`END` . For each `AT`BEGIN` or `AT`END` variable, a
...
performed at the end of the tag for `AT`BEGIN` and `AT`END` and also
before the invocation of a fragment for `AT`BEGIN` . See
link:jsp.html#a2550[See Variable synchronization behavior.] for
details.
* Nested parameters - Use variables with
scope `AT`BEGIN` or `NESTED` . For each `AT`BEGIN` or `NESTED` variable,
a page-scoped attribute is made available in the `JspContext` of the tag
file. The scoped attribute is not initialized. Synchronization is
performed before each fragment invocation for `AT`BEGIN` and `NESTED` ,
and also after the end of the tag for `AT`BEGIN` . See
link:jsp.html#a2550[See Variable synchronization behavior.] for
details.
...
In this example, the `AT`BEGIN` scope is used
...
<%@ variable name-given=”x” scope=”AT`BEGIN” %> +
<%@ taglib prefix=”c” uri=”http://java.sun.com/jsp/jstl/core” %> +
$\{x} <%-- (x == null) --%> +
<c:set var=”x” value=”2”/> +
<jsp:doBody/> +
...
=== Example of AT`BEGIN and name-from-attribute

Like the previous example, in this example
the `AT`BEGIN` scope is used to pass a variable to the tag’s body, and
...
<%@ variable alias=”result” name-from-attribute=”var” scope=”AT`BEGIN”
%> +
<%@ taglib prefix=”c” uri=”http://java.sun.com/jsp/jstl/core” %> +
$\{x} <%-- (x == null) --%> +
$\{result} <%-- (result == null) --%> +
...
In this example, the AT`END scope is used to
...
<%-- /WEB-INF/tags/example.tag --%> +
<%@ variable name-given=”x” scope=”AT`END” %> +
<%@ taglib prefix=”c” uri=”http://java.sun.com/jsp/jstl/core” %> +
$\{x} <%-- (x == null) --%> +
<c:set var=”x” value=”2”/> +
<jsp:doBody/> +
...
| `<x:tag> +
foo +
</x:tag>` | `declare AT`BEGIN variables +
\{ +
declare NESTED variables +
transformation of foo +
} +
declare AT`END variables`
|===
