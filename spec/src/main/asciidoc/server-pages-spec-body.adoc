...
* <<Standard Actions>>
* <<Jakarta_Pages_Documents>>, except for sections that discuss declarations, scriptlets, expressions, and request-time attributes
* <<Goals>> and <<Overview>> of <<Tag Extensions>>
* <<Tag Files>>
* Appendices <<Packaging Jakarta Pages (JSP)>>, <<Changes>>, and <<Glossary>>
...
* Chapters <<Core Syntax and Semantics>>, <<Expression Language>>, <<Jakarta Pages (JSP) Configuration>>, <<Internationalization Issues>> and <<Standard Actions>>
* <<Jakarta_Pages_Documents>>
* <<Valid Jakarta Pages (JSP) Page>> and <<Reserved Names>> of <<Scripting>>
* <<Goals>> and <<_Overview>> of <<Tag Extensions>>
* <<Tag Files>>
* <<Precompilation>> of <<Jakarta Pages (JSP) Container>>
* Appendices <<Packaging Jakarta Pages (JSP)>>, <<Changes>>, and <<Glossary>>
...
* Chapters <<Core Syntax and Semantics>>, <<Expression Language>>, <<Jakarta Pages (JSP) Configuration>>, <<Internationalization Issues>> and
<<Standard Actions>>
* <<Jakarta_Pages_Documents>>
* <<Valid Jakarta Pages (JSP) Page>> and <<Reserved Names>> of <<Scripting>>
* <<Tag Extensions>>
...
* <<What Is a Jakarta Pages (JSP) Page>> and <<Web_Applications>> of
...
<<Jakarta Pages (JSP) Container>> for details). At request time a
...
standard actions like <jsp:expression> were only available in Jakarta Pages (JSP)
...
client request for the target Jakarta Pages (JSP) page. <<Compiling Jakarta Pages (JSP)>> describes
...
<<Packaging Jakarta Pages (JSP)>> contains two examples of Jakarta Pages (JSP) packaged in
WARs:
...
`ServletContext` object. See <<Including Data in Jakarta Pages (JSP)>> for exact
...
See <<Jakarta_Pages_Documents>> for
more details on Jakarta Pages (JSP) documents, and
<<Jakarta Pages (JSP) Configuration>> for more details on
configuration.
...
start of <<Jakarta Pages (JSP) Syntax Grammar>>, and
...
those of XML. See <<Jakarta_Pages_Documents>>.
...
declaration elements. (See <<Jakarta Pages (JSP) Container>>).
...
<<What Is a Jakarta Pages (JSP) Page>> defines the rules for when the
...
it is not XML-compatible. <<Jakarta_Pages_Documents>>
...
to which this Jakarta Pages (JSP) page is transformed (see <<Jakarta Pages (JSP) Container>>). +
...
<<Using Jakarta Pages (JSP) as Error Pages>> for more
details). +
...
<<Deferred_Syntax>>). See
<<Backwards Compatibility>> for more information.
...
`<include-prelude>` and `<include-coda>` elements of the Jakarta Pages (JSP)
...
`language` directive (see <<The `page` Directive>>). In Jakarta Pages (JSP) 4.1, the only
...
. `jakarta.el.MapELResolver` - constructed in read/write mode.
. `jakarta.el.ResourceBundleELResolver`
. `jakarta.el.ListELResolver` - constructed in read/write mode.
. `jakarta.el.ArrayELResolver` - constructed in read/write mode.
. `jakarta.el.RecordELResolver` - always read only.
. `jakarta.el.BeanELResolver` - constructed in read/write mode. +
...
|Jakarta Pages (JSP) Configuration <el-ignored>
...
<<Quoting and Escape Conventions>> and <<Overview of Syntax of Jakarta Pages (JSP) Documents>>
for details.
...
|Jakarta Pages (JSP) Configuration <scripting-invalid>
...
|Jakarta Pages (JSP) Configuration <error-on-el-not-found>
...
current page. See <<Summary of Include Mechanisms in Jakarta Pages (JSP) 4.1>> for a summary of include facilities.
...
and `forward` methods (see Section 9.1.1 in the Servlet 6.2
specification).
...
invocations, Jakarta Pages (JSP) containers must support the use of `<jsp:attribute>`
...
See <<Jakarta Pages (JSP) Syntax Grammar>> for the formal syntax definition of the
...
See <<Jakarta Pages (JSP) Syntax Grammar>> for the formal syntax definition of the `<jsp:invoke>`
standard action.
...
only be used in tag files (see <<Tag Files>>), and must result in a
...
See <<Jakarta Pages (JSP) Syntax Grammar>> for the formal syntax definition of the `<jsp:doBody>`
standard action.
...
via a `<jsp:text>` element before the root element of the Jakarta Pages (JSP) document,
...
of `.jspx` or with `<is-xml>` set to `true` in the Jakarta Pages (JSP) configuration):
...
<<Jakarta_Pages_Documents>> defines several other standard actions that are either
...
of a Jakarta Pages (JSP) document are described in <<XML View>>.
...
<<Examples of Jakarta Pages (JSP) Documents>> for more
...
A Jakarta Pages (JSP) document may or not have a `<jsp:root>`
...
In the Jakarta Pages (JSP) document element `<a href="%= url
...
as described in <<Jakarta Pages (JSP) Document Validation>>. As part of the processing XML quoting will be performed,
...
modify the default rule for when a Jakarta Pages (JSP) document has <jsp:root>. No
...
<<Jakarta Pages (JSP) Fragments>> for more details on Jakarta Pages (JSP)
Fragments.

Because Jakarta Pages (JSP) Fragments do not support
scriptlets, the <body-content> of a SimpleTag cannot be "JSP". A Jakarta Pages (JSP)
...
standard Servlet 6.2 mechanisms to access that information.
...
The EBNF grammar in <<Jakarta Pages (JSP) Syntax Grammar>> describes the syntax of tag
...
page via the `<jsp:forward>` standard action. Just as for Jakarta Pages (JSP), the
...
match the tag file’s Jakarta Pages (JSP) version. See <<Packaging in a JAR>>, and
<<Packaging Directly in a Web Application>>, for how the Jakarta Pages (JSP) version
...
syntax, as described in <<Jakarta_Pages_Documents>>.
...
implicit objects available to the Jakarta Pages (JSP) page. See <<Implicit Objects>>.
...
content they include. See <<Including Data in Jakarta Pages (JSP)>> for the
...
file has no `DOCTYPE` information; see <<Validating an XML View of a Jakarta Pages (JSP) page>>.
...
<<Contracts_between_a_Jakarta_Pages_Page_and_a_Jakarta_Pages_Container>>. We now revisit
...
As shown in <<How_the_Jakarta_Pages_Container_Processes_Jakarta_Pages_Pages>>  the methods
...
directive (see Section <<The `page` Directive>>) in a Jakarta Pages (JSP) page is not
used, the Jakarta Pages (JSP) container can generate any class that satisfies the
contract described in <<How_the_Jakarta_Pages_Container_Processes_Jakarta_Pages_Pages>>
...
<<The_Java_Class_Generated_From_a_Jakarta_Pages_Page>> shows a subclass named
...
<<The_Java_Class_Generated_From_a_Jakarta_Pages_Page>>, except that the class
...
encoding for a Jakarta Pages (JSP) page or tag file. See <<Internationalization Issues>>
...
* https://github.com/jakartaee/pages/issues/245[#245]
  ...
  performance of the `ImportELResolver`. This should significantly improve the
...
* Remove the methods that implemented `ELResolver.getFeatureDescriptors()` as
  those methods have been removed as of EL 6.1.
...
* https://github.com/jakartaee/pages/issues/235[#235]
...
* Deprecate the `isThreadSafe` page directive attribute as the related Servlet
  ...
  specification.
* https://github.com/jakartaee/pages/issues/226[#226]
