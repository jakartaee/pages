...
/**
 * <p>
 * The actions and template data in a Jakarta Pages (JSP) page is written using the JspWriter object that is referenced by the implicit
 ...
 * buffering is the default for Jakarta Pages (JSP) pages.
 * <p>
 ...
 * </ul>
 * <p>
 * Both approaches are valid, and thus both are supported in the Jakarta Pages (JSP) technology. The behavior of a page is controlled by
 * the autoFlush attribute, which defaults to true. In general, Jakarta Pages (JSP) pages that need to be sure that correct and complete
 * data has been sent to their client may want to set autoFlush to false, with a typical case being that where the
 * client is an application itself. On the other hand, Jakarta Pages (JSP) pages that send data that is meaningful even when partially
 ...
 * <p>
 * The "out" implicit variable of a Jakarta Pages (JSP) implementation class is of this type. If the page directive selects
...
abstract public class JspWriter extends java.io.Writer {
     ...
     * <p>
     * This method needs not be invoked explicitly for the initial JspWriter as the code generated by the Jakarta Pages (JSP) container
